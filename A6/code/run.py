...
class Scene15(RNGDataFlow):
    ...
    def __init__(self, dir, name, img_size, meta_dir=None,
        ...
        diff = self.imgs - np.expand_dims(self.imgs.mean(axis=-1), axis=-1)
        self.imgs = diff / np.expand_dims(self.imgs.std(axis=-1), axis=-1)
...
def get_data(datadir, task, train_or_test):
    ...
    if isTrain:
        augmentors = [
            ...
            imgaug.Flip(horiz=False, vert=True, prob=0.3)
            # imgaug.RotationAndCropValid(180, interp=cv2.INTER_LINEAR, step_deg=None)
        ]
    else:
